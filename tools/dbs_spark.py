...
def run(dpath, bpath, fpath, fout, verbose=None, yarn=None):
    """
    ...
    with open(fout, 'w') as ostream:
        ostream.write("dataset,size,evt,date\n")
        for row in rdf:
            ostream.write('%s,%s,%s,%s\n' % (row['d_dataset'], row['size'], row['evt'], row['date']))
...
def main():
    ...
    time0 = time.time()
    results = run(opts.dpath, opts.bpath, opts.fpath, opts.fout, opts.verbose, opts.yarn)
